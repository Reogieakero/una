    ...
    <>
      <section id="cta" aria-labelledby="cta-heading" className="relative overflow-hidden py-20 lg:py-28">
        <div aria-hidden className="absolute inset-0 -z-10">
          <div className="absolute inset-0 bg-gradient-to-b from-cream via-blue-50 to-blue-100/60" />
          <div className="absolute left-1/2 top-10 h-80 w-80 -translate-x-1/2 rounded-full bg-yellow-200/40 blur-[90px]" />
        </div>
            ...
            <div className="rounded-[2rem] border border-white/60 bg-white/75 px-6 py-6 shadow-card backdrop-blur">
              ...
              <ul className="mt-4 flex flex-wrap items-center justify-center gap-2.5" aria-label="Technologies used">
                {stack.map((t) => (
                  <li
                    key={t}
                     className="rounded-full border border-blue-200 bg-cream px-4 py-1.5 text-sm font-bold text-ink-soft"
                  >
                    {t}
                  </li>
                ))}
       ...
       <footer className="border-t border-blue-100 bg-white/70 backdrop-blur" aria-label="Footer">
        <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
            ...
            <nav aria-label="Explore">
              ...
              <ul className="mt-3 space-y-2 text-[15px] font-medium">
                {[
                  ...
                  <li key={href}>
                    <a href={href} className="text-ink-soft hover:text-blue-700 hover:underline">
                      {label}
                    </a>
                  </li>
                ))}
          ...
          <div className="mt-10 flex flex-col items-center justify-between gap-3 border-t border-blue-100 pt-6 text-[13px] text-ink-muted sm:flex-row">
