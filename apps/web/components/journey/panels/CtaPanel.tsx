        ...
        <AnimatedSection delay={0.1} className="mt-6">
          <ul className="flex flex-wrap items-center justify-center gap-2" aria-label="Technologies used">
            {stack.map((t) => (
              <li
                key={t}
                className="rounded-full border border-blue-200 bg-white/80 px-3.5 py-1 text-[13px] font-bold text-ink-soft"
              >
                {t}
              </li>
            ))}
          </ul>
          <div className="mx-auto mt-5 flex max-w-2xl flex-col items-center justify-between gap-2 border-t border-blue-200/70 pt-4 text-[12px] text-ink-muted sm:flex-row">
