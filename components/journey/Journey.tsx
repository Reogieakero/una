    ...
    <main id="j-main" aria-label="DOrSU counseling platform — horizontal tour">
      <div ref={targetRef} className="relative h-[800vh]">
        <div className="sticky top-0 h-dvh overflow-hidden">
          <div
            aria-hidden
            className="pointer-events-none absolute inset-0 bg-[linear-gradient(180deg,#FDF8F1_0%,#EFF6F0_45%,#EFEAF8_80%,#FDF8F1_100%)]"
          />
          <motion.div
            ref={trackRef}
            id="journey-track"
          ...
          >
            <div className="flex items-center gap-3 rounded-full border border-white/70 bg-white/85 py-2 pl-4 pr-3 shadow-card backdrop-blur-xl">
              ...
              <span className="relative h-1 w-24 overflow-hidden rounded-full bg-sage-100 sm:w-32">
                <motion.span
                  style={{ scaleX: progress }}
                  className="absolute inset-0 origin-left rounded-full bg-sage-500"
                />
              </span>
              <div className="flex items-center gap-1.5">
                {PANELS.map((p, i) => (
                  <button
                    ...
                    className="rounded-full p-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sage-500"
                  >
                    <span
                      aria-hidden
                      className={cn(
              ...
              <span className="hidden w-16 text-xs font-bold text-sage-700 sm:block">
