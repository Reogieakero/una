    ...
    >
      <div className="mx-auto max-w-6xl px-4 pt-4 sm:px-6">
        ...
        >
          ...
          <div className="hidden shrink-0 items-center lg:flex">
            {links.map((l) => (
              <a
                key={l.href}
                href={l.href}
                className="whitespace-nowrap rounded-full px-2 py-2 text-[13px] font-semibold text-ink-soft transition-colors hover:bg-sage-50 hover:text-ink xl:px-3 xl:text-sm"
              >
                {l.label}
              </a>
            ))}
          ...
          <button
            className="flex h-10 w-10 items-center justify-center rounded-full border border-sage-200 bg-white lg:hidden"
            onClick={() => setOpen((v) => !v)}
            aria-expanded={open}
            ...
            >
              {links.map((l) => (
                <a
                  ...
                  className="block rounded-2xl px-4 py-3 text-[15px] font-semibold text-ink-soft hover:bg-sage-50"
                >
                  {l.label}
                </a>
              ))}
