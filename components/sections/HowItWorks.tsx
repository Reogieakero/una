...
function StepCard({ s, i }: { s: Step; i: number }) {
  ...
  return (
    <article className="relative flex h-[400px] w-[300px] shrink-0 snap-center flex-col rounded-[1.75rem] border border-white/60 bg-white/92 p-7 pt-9 shadow-card backdrop-blur sm:w-[360px]">
      <span
        aria-hidden
        className="absolute -top-5 left-7 flex h-10 w-10 items-center justify-center rounded-full bg-sage-600 font-display text-base font-bold text-white shadow-soft"
      >
        {i + 1}
      </span>
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <h2 id="how-heading" className="mt-4 text-3xl font-semibold sm:text-[2.6rem]">
            How it works <span className="text-sage-600">for a student.</span>
          </h2>
        ...
        >
          ...
          {active && (
            <div className="relative mx-auto mt-10 flex items-center gap-2" aria-hidden>
              {steps.map((_, i) => (
                <span key={i} className="h-2 rounded-full bg-sage-200" style={{ width: 28 - i * 2, opacity: 1 - i * 0.12 }} />
              ))}
            </div>
          )}
        </div>
